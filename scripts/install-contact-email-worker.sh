...
SERVICE_NAME="${SERVICE_NAME:-aic-contact-email-worker.service}"
TIMER_NAME="${TIMER_NAME:-aic-contact-email-worker.timer}"
ENABLE_TIMER="${ENABLE_TIMER:-0}"
START_TIMER="${START_TIMER:-0}"

case "${ENABLE_TIMER}" in 0|1) ;; *) echo "ENABLE_TIMER must be 0 or 1." >&2; exit 1 ;; esac
