...
type EpisodeReprocessPanelProps = {
  episodeHref: string;
  pageParams: { reprocessQueued?: string; reprocessError?: string };
  reprocessContext: EpisodeReprocessContext | null;
  reprocessLookupError: string;
  trackId: string;
};

function EpisodeReprocessPanel({
  episodeHref,
  pageParams,
  reprocessContext,
  reprocessLookupError,
  trackId,
}: EpisodeReprocessPanelProps) {
  return (
    <section
      className="episode-reprocess-panel"
      id="episode-reprocess"
      aria-labelledby="episode-reprocess-heading"
    >
      <div>
        <p className="eyebrow">Administrator action</p>
        <h4 id="episode-reprocess-heading">Reprocess this episode</h4>
      </div>
      ...
      {reprocessLookupError ? (
        ...
        <p className="empty-state" role="status">
          No matching Strapi episode exists for Track ID {trackId}; reprocessing cannot be queued here.
        </p>
        ...
        <form
          className="editor-form editor-grid editor-grid--two episode-reprocess-panel__form"
          action={queueEpisodeReprocessAction.bind(null, trackId, episodeHref)}
        >
          <p className="muted-copy episode-reprocess-panel__warning">
          ...
          <div className="editor-form__actions">
            <button className="button button--primary" type="submit">Reprocess episode</button>
          </div>
        </form>
      )}
    </section>
...
export default async function EpisodeDetailPage({
  ...
  if (!detail) {
    return (
      ...
      >
        ...
        <Link className="button button--ghost" href="/console/episodes">
          Back to episodes
        </Link>
      </RoutePanel>
    );
  ...
  const episodeHref = `/console/episodes/${encodeURIComponent(detail.episode.trackId)}`;
    ...
    >
      ...
      <section className="detail-section">
        ...
        <TranscriptReader
          ...
          controlsAfter={isAdministrator ? (
            <EpisodeReprocessPanel
              episodeHref={episodeHref}
              pageParams={pageParams}
              reprocessContext={reprocessContext}
              reprocessLookupError={reprocessLookupError}
