...
export default async function LegacyEpisodeDetailPage({ params, searchParams }: {
  params: Promise<{ trackId: string }>;
  searchParams: Promise<ConsoleRedirectSearchParams>;
}) {
  await requireInternalReadConsoleUser();
  const { trackId } = await params;
  redirect(consolePathWithSearchParams(
    `/console/episodes/${encodeURIComponent(trackId)}`,
    await searchParams,
  ));
}
